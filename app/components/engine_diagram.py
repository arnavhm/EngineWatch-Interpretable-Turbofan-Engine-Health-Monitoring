...
def render_engine_diagram(
    engine_df: pd.DataFrame, artifacts: dict, config: dict, dataset_id: str
) -> None:
    """
    ...
    for axis in axes_to_render:
        ...
        for s in axis_sensors:
            ...
            if col_name in last_row:
                s_num = s.split("_")[-1]
                s_key = f"s{s_num}"
                sensor_contributions[s_key] = sensor_contributions.get(
                    s_key, 0.0
                ) + float(last_row[col_name])
    ...
    for module_key, data in module_heat.items():
        base_match = (
            f'<g id="module-{module_key}" data-module="{module_key}" class="module">'
        )
