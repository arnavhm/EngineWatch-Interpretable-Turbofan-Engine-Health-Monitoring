...
def get_agentic_tools(
    ...
    def simulate_aog_cost(revenue_loss_per_day_rs_lakh: float = None, aog_cost_per_event_rs_cr: float = None) -> str:
        """
        ...
        if revenue_loss_per_day_rs_lakh is not None:
            sim_config["aog_simulator"]["revenue_per_day_rs_lakh"] = float(revenue_loss_per_day_rs_lakh)
        if aog_cost_per_event_rs_cr is not None:
            sim_config["aog_simulator"]["aog_cost_per_event_rs_cr"] = float(aog_cost_per_event_rs_cr)
    ...
    def query_fleet_status(sort_by: str = "risk_score", top_n: int = 5) -> str:
        ...
        allowed_sort = {"risk_score", "health_index", "HI_velocity", "HI_variability", "cycle"}
        ...
        sorted_fleet = latest_fleet.sort_values(sort_by, ascending=ascending).head(top_n)
        
        cols = ["unit", "cycle", "risk_state", "risk_score", "health_index", "HI_velocity", "HI_variability"]
