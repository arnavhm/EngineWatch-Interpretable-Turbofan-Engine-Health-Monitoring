...
def main() -> None:
    ...
    for i, row in last.iterrows():
        ...
        if compute_ci:
            ci_lower, ci_upper, ci_std = _compute_rf_ci(
                rf_model, features.values.reshape(1, -1), predicted_rul
            )
        ...
        pred_dict = {
            ...
            "rmse": float(
                artifacts.evaluation_metrics[artifacts.best_model_name]["rmse"]
            ),
        }
    ...
    print(
        f"[artifacts] trajectory_cache_FD001.pkl saved ({len(trajectory_cache)} engines)"
    )
